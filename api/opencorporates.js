...
// Söker på bolagsnamn via OpenCorporates API
// Anropas från frontend när användaren skriver ett företagsnamn
//
// GET /api/opencorporates?q=Getinge&jurisdiction=se
//
// Miljövariabler som krävs i Vercel:
...
export default async function handler(req, res) {
  ...
  if (!q || q.trim().length < 2) {
    return res.status(400).json({ error: 'Sökfråga krävs (minst 2 tecken)' });
  }

  const token = process.env.OPENCORPORATES_API_TOKEN;
  if (!token) {
    return res.status(500).json({ error: 'API-token saknas' });
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error('OpenCorporates fel:', err);
      return res.status(502).json({ error: 'Fel från OpenCorporates', details: err });
    }

    const data = await response.json();
    const companies = data?.results?.companies || [];

    // Formatera till något enkelt för frontend
    const results = companies.map(({ company: c }) => ({
      namn:         c.name,
      orgnr:        c.company_number?.replace(/\D/g, '') || null, // bara siffror
...
// Formatera org.nr med bindestreck: 5560401536 -> 556040-1536
