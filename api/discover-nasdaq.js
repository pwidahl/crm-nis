  ...
  for (const b of bolag) {
    try {
      ...
      if (existing?.id) {
        ...
        if (error) { const { data: dup } = await supabase.from('companies').select('id').eq('user_id', userId).ilike('namn', b.namn).maybeSingle(); if (!dup?.id) errors.push(`${b.namn}: ${error.message}`); }
        else skapade++;
      }
    } catch (err) {
