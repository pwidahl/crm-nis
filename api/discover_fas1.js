...
export default async function handler(req, res) {
  ...
  // Cron-läge (GET): kör de proaktiva kris-/bolagskällorna för alla användare
  if (req.method === 'GET') {
    ...
    const cronKallor = ['bolagsverket_changes', 'bolagsverket_new', 'upphandling', 'mynewsdesk', 'fi'];
    const resultat = [];
    for (const uid of userIds) {
      for (const k of cronKallor) {
        try {
          const fakeRes = { status: () => ({ json: (x) => x }) };
          const r = await runKalla(supabase, uid, k, fakeRes, req);
          resultat.push({ userId: uid, kalla: k, ...(r || {}) });
        } catch (err) {
          resultat.push({ userId: uid, kalla: k, error: err.message });
        }
      }
    }
    return res.status(200).json({ message: 'discover_fas1 cron klar', resultat });
  }
  ...
  return runKalla(supabase, userId, source, res, req);
}

async function runKalla(supabase, userId, source, res, req) {
