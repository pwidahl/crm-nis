  ...
  for (const b of bolag) {
    try {
      ...
      if (ex?.id) {
        ...
        if (error) { const { data: dup } = await sb.from('companies').select('id').eq('user_id', userId).ilike('namn', b.namn).maybeSingle(); if (!dup?.id) errors.push(`${b.namn}: ${error.message}`); } else skapade++;
      }
...
async function findOrCreate(sb, userId, company) {
  ...
  if (error) { const { data: dup } = await sb.from('companies').select('id').eq('user_id', userId).ilike('namn', namn).maybeSingle(); if (dup?.id) return { id: dup.id, created: false }; return { error: `${namn}: ${error.message}` }; }
